...
public sealed class UpdateOneProductService(
...
) : IUpdateOneProductService
{
    ...
    )
    {
        ...
        var productFoundById = await productsRepository.FindOneAsync(
            filter: x => x.Id == command.ProductDto.Id && x.IsDeleted == false,
            cancellationToken: cancellationToken
        );

        return new UpdateOneProductServiceResponse(
            ProductFoundById: productFoundById
        );
    }
}

public record UpdateOneProductServiceResponse(
