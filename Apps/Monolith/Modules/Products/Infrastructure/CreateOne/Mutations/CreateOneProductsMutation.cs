...
public sealed class CreateOneProductsMutation
{
    [Authorize(Policy = ProductsPolicies.CreateOneProductsPolicy)]
    public async Task<ProductModel> CreateOneProducts(
    ...
    )
    {
        ...
        {
            CreatedBy = userId,
            Variants = input.Variants.Select(
                v => v with
                {
                    CreatedBy = userId,
                    CreatedAtTimezone = input.CreatedAtTimezone,
                    ProductId = input.Id,
                    VariantsOnAttributes = v.VariantsOnAttributes.Select(
                        voa => voa with
                        {
                            CreatedBy = userId,
                            CreatedAtTimezone = input.CreatedAtTimezone,
