...
) : IRequestHandler<CreateOneWarehouseCommand, WarehouseModel>
{
    ...
    private readonly ICreateOneWarehouseService _createOneWarehouseService =
        createOneWarehouseService;
    ...
    )
    {
        ...
        var aggregateRoot = WarehouseAggregateRoot.CreateOne(command: command, data: fetchResponse);

        var warehouseModel = _mapper.Map<WarehouseModel>(source: aggregateRoot);

        await warehousesRepository.CreateOneAsync(
