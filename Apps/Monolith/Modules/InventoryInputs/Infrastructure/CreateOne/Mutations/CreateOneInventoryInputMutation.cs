...
public sealed class CreateOneInventoryInputMutation
{
    ...
    )
    {
        ...
        var inventoryInputDto = mapper.Map<CreateOneInventoryInputDto>(source: input with { CreatedBy = userId });

        return await mediator.Send(new CreateOneInventoryInputCommand(InventoryInputDto: inventoryInputDto));
    }
}
