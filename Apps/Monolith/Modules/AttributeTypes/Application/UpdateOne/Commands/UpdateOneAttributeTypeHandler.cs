...
) : IRequestHandler<UpdateOneAttributeTypeCommand, AttributeTypeModel>
{
    ...
    private readonly IUpdateOneAttributeTypeService _updateOneAttributeTypeService = updateOneAttributeTypeService;

    public async Task<AttributeTypeModel> Handle(UpdateOneAttributeTypeCommand command, CancellationToken cancellationToken)
    {
        var fetchResponse = await _updateOneAttributeTypeService.ExecuteAsync(command: command, cancellationToken: cancellationToken);
