...
public sealed class CreateOneAttributeTypesMutation
{
    [Authorize(Policy = AttributeTypesPolicies.CreateOneAttributeTypesPolicy)]
    public async Task<AttributeTypeModel> CreateOneAttributeTypes(
    ...
    )
    {
        ...
        var attributeTypeDto = mapper.Map<CreateOneAttributeTypeDto>(source: input with
        {
            ValuePattern = Encoding.UTF8.GetString(Convert.FromBase64String(input.ValuePattern)),
            MeasuringUnitPattern = Encoding.UTF8.GetString(Convert.FromBase64String(input.MeasuringUnitPattern)),
            CreatedBy = userId
        });

        return await mediator.Send(
            new CreateOneAttributeTypeCommand(
                attributeTypeDto
            )
        );
    }
