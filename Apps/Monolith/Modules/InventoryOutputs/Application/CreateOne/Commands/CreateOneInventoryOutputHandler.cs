...
{
    private readonly ICreateOneInventoryOutputService _createOneInventoryOutputService = createOneInventoryOutputService;
    ...
    )
    {
        ...
        var inventoryOutputModel = _mapper.Map<InventoryOutputModel>(
            aggregateRoot.InventoryOutputs.FirstOrDefault(x => x.Id == command.InventoryOutputDto.Id)
        );
        var inventoryModel = _mapper.Map<InventoryModel>(
            aggregateRoot.Inventories.FirstOrDefault(x => x.Id == command.InventoryOutputDto.InventoryId)
        );

        await inventoryOutputsRepository.CreateOneAsync(entity: inventoryOutputModel, cancellationToken: cancellationToken);
