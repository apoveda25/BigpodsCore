...
public class AttributeModelConfiguration : IEntityTypeConfiguration<AttributeModel>
{
    public void Configure(EntityTypeBuilder<AttributeModel> builder)
    {
        ...
        builder.Property(propertyExpression: x => x.Value).IsRequired().HasMaxLength(maxLength: 255);

        builder.Property(propertyExpression: x => x.MeasuringUnit).IsRequired().HasMaxLength(maxLength: 255);
        ...
        builder.Property(propertyExpression: x => x.AttributeTypeId).UseCollation(collation: "utf8mb4_general_ci");

        builder.HasIndex(indexExpression: p => new { p.Value, p.MeasuringUnit, p.AttributeTypeId });
    }
}