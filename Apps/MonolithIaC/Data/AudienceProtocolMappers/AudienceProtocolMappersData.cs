...
public sealed record AudienceProtocolMapper(
    string Name,
    Pulumi.Keycloak.OpenId.AudienceProtocolMapperArgs Config
);

public class AudienceProtocolMappersData
{
    public static string AudienceBigpodsGUIAPIClientName { get; } =
        $"{ClientScopesData.AudienceName}:{ClientsData.BigpodsMonolithAPIClientName}";
    ...
    )
    {
        ...
        string audienceBigpodsGUIAPIClientNameKebabCase = StringUtils.ToKebabCase(
            AudienceBigpodsGUIAPIClientName
        );

        return new Dictionary<string, AudienceProtocolMapper>()
        {
            [AudienceBigpodsGUIAPIClientName] = new AudienceProtocolMapper(
