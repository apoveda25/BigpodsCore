...
{
    public static string AdminUserToAdminRoleName { get; } = $"{UsersData.AdminName}:{RolesData.AdminName}";
    ...
    )
    {
        ...
        string adminUserToAdminRoleNameKebabCase = StringUtils.ToKebabCase(AdminUserToAdminRoleName);

        return new Dictionary<string, UserRoles>
        {
            [AdminUserToAdminRoleName] = new UserRoles(Name: $"user-roles:{adminUserToAdminRoleNameKebabCase}", Config: new()
            {
