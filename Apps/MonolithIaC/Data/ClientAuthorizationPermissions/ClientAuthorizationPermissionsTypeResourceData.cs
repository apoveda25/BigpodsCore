...
public partial class ClientAuthorizationPermissionsData
{
    ...
    public static string ResourceInventoryInputsName { get; } = ClientAuthorizationResourcesData.InventoryInputsName;
    public static string ResourceInventoryOutputsName { get; } = ClientAuthorizationResourcesData.InventoryOutputsName;
    ...
    )
    {
        ...
        var inventoryInputsPolicies = policies.Where(policy => policy.Key.Contains(ClientAuthorizationResourcesData.InventoryInputsName)).Select(policy => policy.Value.Name);
        var inventoryOutputsPolicies = policies.Where(policy => policy.Key.Contains(ClientAuthorizationResourcesData.InventoryOutputsName)).Select(policy => policy.Value.Name);
        ...
        string resourceInventoryInputsNameKebabCase = StringUtils.ToKebabCase(ResourceInventoryInputsName);
        string resourceInventoryOutputsNameKebabCase = StringUtils.ToKebabCase(ResourceInventoryOutputsName);
        ...
        return new Dictionary<string, ClientAuthorizationPermission>
        {
            ...
            [ResourceInventoryInputsName] = new ClientAuthorizationPermission(Name: $"permissions:{resourceInventoryInputsNameKebabCase}", Config: new()
            {
                RealmId = bigpodsRealm.Id.Apply(id => id),
                ResourceServerId = bigpodsMonolithAPIClient.Id.Apply(id => id),
                DecisionStrategy = "UNANIMOUS",
                ...
                Resources = resources[ClientAuthorizationResourcesData.InventoryInputsName].Name.Apply(name => new[] { name }),
                Type = "resource"
            }),
            [ResourceInventoryOutputsName] = new ClientAuthorizationPermission(Name: $"permissions:{resourceInventoryOutputsNameKebabCase}", Config: new()
            {
                RealmId = bigpodsRealm.Id.Apply(id => id),
                ResourceServerId = bigpodsMonolithAPIClient.Id.Apply(id => id),
                DecisionStrategy = "UNANIMOUS",
                ...
                Resources = resources[ClientAuthorizationResourcesData.InventoryOutputsName].Name.Apply(name => new[] { name }),
                Type = "resource"
            }),
