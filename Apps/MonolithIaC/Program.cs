...
return await Deployment.RunAsync(() =>
{
    ...
    var realmsCreated = RealmsData
        ...
        .ToDictionary(
            item => item.Key,
            item => RealmFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientsCreated = ClientsData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var usersCreated = UsersData
        ...
        .ToDictionary(
            item => item.Key,
            item => UserFactory.Build(item.Value.Name, item.Value.Config)
        );

    // Get Keycloak default roles
    var gettingRoles = RolesData.InvokeRoles(realms: realmsCreated);
    ...
    var rolesCreated = RolesData
        ...
        .ToDictionary(
            item => item.Key,
            item => RolesFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var userRolesCreated = UsersRolesData
        ...
        .ToDictionary(
            item => item.Key,
            item => UserRolesFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientAuthorizationResourcesCreated = ClientAuthorizationResourcesData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientAuthorizationResourceFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientRolePoliciesCreated = ClientRolePoliciesData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientRolePolicyFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientAuthorizationPermissionsCreated = ClientAuthorizationPermissionsData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientAuthorizationPermissionFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientScopesCreated = ClientScopesData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientScopesFactory.Build(item.Value.Name, item.Value.Config)
        );

    // Create Keycloak audience protocol mappers
    var audienceProtocolMappersCreated = AudienceProtocolMappersData
        .GetAudienceProtocolMappers(
            realms: realmsCreated,
            clients: clientsCreated,
            clientScopes: clientScopesCreated
        )
        .AsParallel()
        .ToDictionary(
            item => item.Key,
            item => AudienceProtocolMapperFactory.Build(item.Value.Name, item.Value.Config)
        );
    ...
    var clientDefaultScopesCreated = ClientDefaultScopesData
        ...
        .ToDictionary(
            item => item.Key,
            item => ClientDefaultScopeFactory.Build(item.Value.Name, item.Value.Config)
        );

    return new Dictionary<string, object?>
    {
        ["realmsId"] = Output.All(
            realmsCreated.Select(realm => Output.All(realm.Value.Id).Apply(realm => realm))
        ),
        ["clients"] = Output.All(
            clientsCreated.Select(client =>
                Output.All(client.Value.Id, client.Value.ClientId).Apply(client => client)
            )
        ),
        ["usersIds"] = Output.All(
            usersCreated.Select(user =>
                Output
                    .All(
                        user.Value.Id,
        ...
        ["rolesIds"] = Output.All(
            rolesCreated.Select(role =>
                Output.All(role.Value.Id, role.Value.Name, role.Value.RealmId).Apply(role => role)
            )
        ),
        ["userRolesIds"] = Output.All(
            userRolesCreated.Select(userRole =>
                Output
                    .All(
                        userRole.Value.Id,
        ...
        ["clientAuthorizationScopesIds"] = Output.All(
            clientAuthorizationScopesCreated.Select(scope =>
                Output.All(scope.Value.Id, scope.Value.Name).Apply(scope => scope)
            )
        ),
        ["clientAuthorizationResourcesIds"] = Output.All(
            clientAuthorizationResourcesCreated.Select(resource =>
                Output.All(resource.Value.Id, resource.Value.Name).Apply(resource => resource)
            )
        ),
        ["clientRolePoliciesIds"] = Output.All(
            clientRolePoliciesCreated.Select(policy =>
                Output.All(policy.Value.Id, policy.Value.Name).Apply(policy => policy)
            )
        ),
        ["clientAuthorizationPermissionsIds"] = Output.All(
            clientAuthorizationPermissionsCreated.Select(permission =>
                Output
                    .All(permission.Value.Id, permission.Value.Name)
        ...
        ["clientScopesIds"] = Output.All(
            clientScopesCreated.Select(clientScope =>
                Output
                    .All(clientScope.Value.Id, clientScope.Value.Name, clientScope.Value.RealmId)
